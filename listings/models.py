...
from main.libs.main_libs import get_headshot_image, get_image_format, get_coordinates
...
class Listing(models.Model):
    listing_type = models.ForeignKey(
        ListingType, on_delete=models.PROTECT, verbose_name=_("Listing type")
    )
    realtor = models.ForeignKey(
        "accounts.Realtor", on_delete=models.DO_NOTHING, verbose_name=_("Realtor")
    )
    ...
    free_from = models.DateField(
        default=datetime.now, blank=True, verbose_name=_("Free from")
    )
    ...
    def get_address(self):
        return (
            f"{self.address.street} {self.address.hn}, {self.address.city}"
    ...
    def get_images(self):
        ...
        return (
            self.listingimage_set.count() + 1
            if self.image
            else self.listingimage_set.count()
        )
...
class ListingImage(models.Model):
    listing = models.ForeignKey(
        Listing, default=None, on_delete=models.DO_NOTHING, verbose_name=_("Listing")
    )
    image = models.ImageField(
        default=None,
        upload_to=listing_dir_path,
        null=True,
        blank=True,
        verbose_name=_("Image"),
    )
    short_description = models.CharField(
        max_length=255, verbose_name=_("Short description")
    )
