...
def user_contact(request):
    if request.method == "POST":
        ...
        if request.user.is_authenticated:
            ...
            if has_contacted:
                messages.error(
                    request, _("You have already made an inquiry " "for this listing")
                )
                return redirect("listing", listing_id)

        contact = Contact(
            listing_id=listing_id, phone=phone, message=message, user_id=user_id
        )
...
def anonymous_contact(request):
    if request.method == "POST":
        ...
        if request.user.is_authenticated:
            user_id = request.user.id
            has_contacted = Contact.objects.all().filter(
                listing_id=listing_id, first_name=first_name, last_name=last_name
            )
            if has_contacted:
                messages.error(
                    request, _("You have already made an inquiry for this " "listing")
...
def chat_message(request):
    if request.method == "POST":
        ...
        obj = ChatMessage(contact_id=contact_id, message=message, user_id=user_id)
...
class MessageHistoryListView(DetailView):
    model = Contact
    template_name = "connects/messages_history.html"
