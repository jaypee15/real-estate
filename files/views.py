...
class UserDocumentView(DetailView):
    model = Listing
    template_name = "files/documents.html"

    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context["contact"] = Contact.objects.get(
            listing_id=self.object.id, user_id=self.request.user.id
        )
        ...
        context["page_description"] = _(
            "Realty Management." "Upload and manage documents "
        )
        return context


class ListingDocumentView(TemplateView):
    template_name = "files/user-documents.html"
    ...
    def get_context_data(self, **kwargs):
        ...
        context["subtitle"] = format_html(
            f'<i class="fas fa-map-marker"></i>' f" {self.object.listing.address}"
        )
        # SEO
        context["page_title"] = _("Documents")
        context["page_description"] = _(
            "Realty Management." "Manage documents related to specific properties"
        )
        return context
